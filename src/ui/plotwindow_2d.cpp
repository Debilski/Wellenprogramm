...
void Waveprogram2DPlot::setTitle()
{
    if (latticeController_->isValid()) {
        setWindowTitle(QString::fromUtf8(
                           "‘%1’: Simulate a field size of %2×%3 on a lattice of %4×%5 ")
                           .arg(
                               latticeController_->modelTitle())
                           .arg(latticeController_->sizeX())
                           .arg(
                               latticeController_->sizeY())
                           .arg(latticeController_->latticeSizeX())
                           .arg(
...
Waveprogram2DPlot::Waveprogram2DPlot(QMainWindow* parent) : QMainWindow(parent), parent(parent)
{
...
void Waveprogram2DPlot::setUpModelProperties()
{
    ...
    if (latticeController_->lattice()) {
        parameterWidget->setWindowTitle(QString("%1 Properties").arg(latticeController_->lattice()->modelName().c_str()));
    }
    ...
    //    delete parameterWidgetFormLayout;
    if (latticeController_->lattice()) {
        adaptationParameterWidget->setWindowTitle(QString("%1 Adaptation Properties").arg(latticeController_->lattice()->modelName().c_str()));
    }
...
void Waveprogram2DPlot::updatePlotAnnotations()
{
    ...
    foreach (T_listPairs cb, bufferMap) {
        ...
        foreach (T_pair sp, cb) {
            if (lastx > 0 && (abs(lastx - sp.first.x) > 25 || abs(lasty - sp.first.y) > 25)) {
...
void Waveprogram2DPlot::replot()
{
    ...
    if (showClusterIds_ && QLatin1String(plotTabWidget->currentWidget()->metaObject()->className()) == QLatin1String("PlotView")) {
