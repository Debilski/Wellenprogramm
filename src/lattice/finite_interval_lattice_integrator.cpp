    ...
    int x, int y)
{
    ...
    return -20.0 * a(x, y) + 4.0 * (a(x - 1, y) + a(x + 1, y) + a(x, y - 1) + a(x, y + 1)) + a(x + 1, y + 1) + a(
                                                                                                                   x - 1, y + 1)
        + a(x + 1, y - 1) + a(x - 1, y - 1);
}
...
inline double FiniteIntervalIntegrator<T_model>::my_laplacian_periodic(blitz::Array<double, 2>& a, int x, int y)
{
    //return - 4 * a( x, y ) + a( x - 1, y ) + a( x + 1, y ) + a( x, y - 1 )
    //    + a( x, y + 1 );
    return -20.0 * a(Base::indexToX(Base::indexPeriodic(x, y)), Base::indexToY(Base::indexPeriodic(x, y))) + 4.0 * (a(Base::indexToX(Base::indexPeriodic(x - 1, y)), Base::indexToY(Base::indexPeriodic(x - 1, y))) + a(Base::indexToX(Base::indexPeriodic(x + 1, y)), Base::indexToY(Base::indexPeriodic(x + 1, y))) + a(Base::indexToX(Base::indexPeriodic(x, y - 1)), Base::indexToY(Base::indexPeriodic(x, y - 1))) + a(Base::indexToX(Base::indexPeriodic(x, y + 1)), Base::indexToY(Base::indexPeriodic(x, y + 1))))
        + 1.0 * (a(
