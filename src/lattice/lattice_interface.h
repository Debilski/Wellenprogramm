...
struct GeneralComponentSystem
{
    ...
    GeneralComponentSystem(blitz::Array<double, 1> vector) : components(vector), number_of_Variables(components.size())
    {
    }
    GeneralComponentSystem(const std::vector<double>& vector) : components(vector.size()), number_of_Variables(components.size())
    {
        for (uint i = 0; i < vector.size(); ++i)
            components(i) = vector[i];
    }
    GeneralComponentSystem(int size) : components(size), number_of_Variables(components.size())
    {
        components = 0;
    }
    GeneralComponentSystem() : components(1), number_of_Variables(components.size())
    {
        components = 0;
    }
...
struct ComponentSystem
{
    ...
    ComponentSystem(T_vector vector) : components(vector)
    {
...
{
    HodgkinHuxleyComponentSystem() : ComponentSystem<4>(T_vector(0, 0, 0, 0))
    {
    }
    HodgkinHuxleyComponentSystem(double V, double n, double m, double h) : ComponentSystem<4>(T_vector(V, n, m, h))
    {
    }
    HodgkinHuxleyComponentSystem(T_vector vector) : ComponentSystem<4>(vector)
    {
    }
    HodgkinHuxleyComponentSystem(const GeneralComponentSystem& generalComponentSystem) : ComponentSystem<4>(generalComponentSystem)
    {
    }
...
{
    ThreeComponentSystem() : ComponentSystem<3>(T_vector(0, 0, 0))
    {
    }
    ThreeComponentSystem(double u, double v, double w) : ComponentSystem<3>(T_vector(u, v, w))
    {
    }
    ThreeComponentSystem(T_vector vector) : ComponentSystem<3>(vector)
    {
    }
    ThreeComponentSystem(const GeneralComponentSystem& generalComponentSystem) : ComponentSystem<3>(generalComponentSystem)
    {
    }
...
{
    TwoComponentSystem() : ComponentSystem<2>(T_vector(0, 0))
    {
    }
    TwoComponentSystem(double u, double v) : ComponentSystem<2>(T_vector(u, v))
    {
    }
    TwoComponentSystem(T_vector vector) : ComponentSystem<2>(vector)
    {
    }
    TwoComponentSystem(const GeneralComponentSystem& generalComponentSystem) : ComponentSystem<2>(generalComponentSystem)
    {
    }
...
{
    OneComponentSystem(double u = 0) : ComponentSystem<1>(T_vector(u))
    {
    }
    OneComponentSystem(T_vector vector) : ComponentSystem<1>(vector)
    {
    }
    OneComponentSystem(const GeneralComponentSystem& generalComponentSystem) : ComponentSystem<1>(generalComponentSystem)
    {
    }
    const double& u() const
    {
...
#define _COMPONENT_SYSTEM_CONSTRUCTORS(system_number, system_name) \
    ...
    system_name(const GeneralComponentSystem& generalComponentSystem) : ComponentSystem<system_number>(generalComponentSystem) {}
...
struct Parameter
{
    Parameter(T initial = 0, const std::string& name = std::string(), T min = 0, T max = 500,
        int decimals = 3) : name(name), value_(initial), min_(min), max_(max), stepSizeHint_(0.05), decimals_(decimals)
    {
    }
    Parameter(Parameter const& P) : name(P.name), value_(P.value_), min_(P.min_), max_(P.max_), stepSizeHint_(P.stepSizeHint_), decimals_(P.decimals_)
    {
    }
    ~Parameter()
    {
...
struct Defect
{
    ...
    Defect(const Defect<GeneralComponentSystem>& d) : centre(d.centre), radius(d.radius), value(Components(d.value)), boundaryCondition(d.boundaryCondition)
    {
    }
};
...
public:
    ...
    inline ComponentInfo& setPhysicalUnitSymbol(std::string name)
    {
        physicalUnitSymbol_ = name;
        return *this;
    }
    inline std::string physicalQuantity() const { return physicalQuantity_; }
    inline ComponentInfo& setPhysicalQuantity(std::string name)
    {
        physicalQuantity_ = name;
        return *this;
    }
 ...
 */
inline LatticeInterface::LatticeInterface() : LatticeGeometry()
{
}
