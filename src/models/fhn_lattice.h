...
template <>
struct Metainfo<FhnLattice>::NoiseMapping<secondComponent>
{
    enum
    {
        ...
        if (num == 4) {
            ...
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
                    ...
                    d.centre.x = x * cos(30. * 2. * M_PI / 360.) - y * sin(30. * 2. * M_PI / 360.);
                    d.centre.y = x * sin(30. * 2. * M_PI / 360.) + y * cos(30. * 2. * M_PI / 360.);
                    d.radius = var_size();
                    addDefect(d);
                }
            }
...
template <>
struct Metainfo<GameOfLife>::NoiseMapping<firstComponent>
{
    enum
    {
        value = firstComponent
...
template <>
struct Metainfo<FhnHeartLattice>::NoiseMapping<secondComponent>
{
    enum
    {
        value = firstComponent
...
public:
    ...
    FhnHeartLattice() : epsilon(0.1, "epsilon", 0.0, 100.0), gamma(0.1, "gamma", 0.0, 100.0), gamma_high(0.55, "γ high"), gamma_low(0.1, "γ low"), gamma_spacing(0.001, "γ spacing"), waveSize_threshold(200, "wavesize"), var_intensity(0, "intensity", 0, 100), var_size(3, "size", 0, 100), var_speed(0, "speed", 0, 100)
